...
class KayaVelTask(RLTask):
    def __init__(
        ...
        self.door_passed = torch.zeros(self._num_envs, device=self._device)
        self.previous_door_passed = torch.zeros(self._num_envs, device=self._device)
        self.no_obs_ics = None

        self.extras_success = deque([], maxlen=self.num_envs)
        self.extras_collisions = deque([], maxlen=self.num_envs)

        self.door_width = 4.0
        self.extras['door width'] = self.door_width
    ...
    def get_observations(self) -> dict:
        ...
        # update if door is passed or not
        self.previous_door_passed = self.door_passed.clone()
        positions = self.positions - self.initial_root_pos
        target_positions = self.target_positions - self.initial_root_pos
        diff = abs(target_positions - positions)[:, 1]
        self.door_passed = torch.where(diff < 2.5, 1.0, 0.0)
    ...
    def reset_idx(self, env_ids):
        ...
        # set doors size
        door_widths = torch.ones(num_resets,) * self.door_width #torch.rand(num_resets,) * 0.75 + 1.25
        if self.test:
            for i in range(num_resets):
                id = env_ids[i].item()
                door_widths[i] = self.door_width  + 0.25 - id
                
        lengths = 4.9 - door_widths/2

        l_positions, _ = self._home.get_world_poses(indices=env_ids)
        r_positions, _ = self._home.get_world_poses(indices=env_ids)
        ...
        # reset if door is passed
        self.door_passed[env_ids] = 0.0
        self.previous_door_passed[env_ids] = 0.0
    ...
    def post_reset(self):
        ...
        if num > 0:
            ...
            #self.initial_root_pos[ics] = home_pos
            self._home.set_world_poses(home_pos, indices=ics)
            self._l_doors.set_world_poses(home_pos, indices=ics)
            self._r_doors.set_world_poses(home_pos, indices=ics)
            self.no_obs_ics = ics
    ...
    def calculate_metrics(self) -> None:
        ...
        # reward for passing the door
        r_door = self._task_cfg['reward']['r_door']
        newly_passed = torch.where(self.door_passed.bool() & ~self.previous_door_passed.bool(), r_door, 0.0)
        back_passed = torch.where(~self.door_passed.bool() & self.previous_door_passed.bool(), -r_door, 0.0)
        # do not give any reward for envs without obstacles
        if self.no_obs_ics is not None:
            newly_passed[self.no_obs_ics] = 0.0
            back_passed[self.no_obs_ics] = 0.0
        ...
        if self.csv_save:
            ...
            new_row = [self.positions[0,0], self.positions[0,1], round(self.time_passed, 1, self._episode)]
            pdb.set_trace()
    ...
    def reset_targets(self) -> None:
        ...
        if torch.any(self.goal_reached):
            ...
            self.reached_targets += indices[0].shape[0]
            robot_positions = self.positions[indices] - self.initial_root_pos[indices]
            old_targets = self.target_positions[indices] - self.initial_target_pos[indices]
            target_positions = -old_targets
            self._targets.set_world_poses(self.initial_target_pos[indices] + target_positions, indices=indices[0])
            # reset if door is passed
            self.door_passed[indices[0]] = 0.0
            self.previous_door_passed[indices[0]] = 0.0
        self.door_width = max(1.0, 4.0 - int(self.reached_targets/64) * 0.1)
        self.extras['door width'] = self.door_width