...
class KayaTask(RLTask):
    def __init__(
        ...
        self._max_episode_length = 2400
        self.target_margin = 0.35
    
        self.collision_range = 0.75 # collision termination
    ...
    def set_up_scene(self, scene) -> None:
        ...
        #    prim_paths_expr="/World/envs/.*/obs.*",
        #    name="obstacle_view"
        #)
        #scene.add(obstacles)
    ...
    def get_walls(self):
        POS = self._task_cfg["env"]["envSpacing"]/2
        LENGTH = POS*2
        HEIGHT = 0.5
        DEPTH = .1
        cube = DynamicCuboid(
            prim_path=self.default_zero_env_path + "/obs0",
            position = np.array([-POS, 0, HEIGHT/2]),
            scale=[DEPTH, LENGTH, HEIGHT],
            #size=np.array([0.5, 4, 1])
        )
        #self._sim_config.apply_articulation_settings("obstacle", get_prim_at_path(cube.prim_path),
        #                                             self._sim_config.parse_actor_config("obstacle"))
        #cube.set_collision_enabled(True)
        #pdb.set_trace()
        FixedCuboid(
            prim_path=self.default_zero_env_path + "/obs1",
            position = np.array([POS, 0, HEIGHT/2]),
            scale=[DEPTH, LENGTH, HEIGHT],
            #size=np.array([0.5, 4, 1])
        )
        FixedCuboid(
            prim_path=self.default_zero_env_path + "/obs2",
            position = np.array([0, POS+DEPTH/2, HEIGHT/2]),
            scale=[LENGTH+DEPTH, DEPTH, HEIGHT],
            #size=np.array([0.5, 4, 1])
        )
        FixedCuboid(
            prim_path=self.default_zero_env_path + "/obs3",
            position = np.array([0, -POS-DEPTH/2, HEIGHT/2]),
            scale=[LENGTH+DEPTH, DEPTH, HEIGHT],
            #size=np.array([0.5, 4, 1])
        )
    ...
    def get_obstacles(self):
        ...
        WIDTH = .2
        FixedCylinder(
            prim_path=self.default_zero_env_path + "/obs4",
            position=[POS, POS, HEIGHT/2],
            scale=[WIDTH, WIDTH, HEIGHT]
        )
        FixedCylinder(
            prim_path=self.default_zero_env_path + "/obs5",
            position=[-POS, POS, HEIGHT/2],
            scale=[WIDTH, WIDTH, HEIGHT]
        )
        FixedCylinder(
            prim_path=self.default_zero_env_path + "/obs6",
            position=[POS, -POS, HEIGHT/2],
            scale=[WIDTH, WIDTH, HEIGHT]
        )
        FixedCylinder(
            prim_path=self.default_zero_env_path + "/obs7",
            position=[-POS, -POS, HEIGHT/2],
            scale=[WIDTH, WIDTH, HEIGHT]
        )
        FixedCylinder(
            prim_path=self.default_zero_env_path + "/obs8",
    ...
    def get_observations(self) -> dict:
        ...
        for i in range(self._num_envs):
            ...
            #pdb.set_trace()
            #print(np.min(reshaped_array), np.max(reshaped_array))
    ...
    def calculate_metrics(self) -> None:
        ...
        self.goal_reached = torch.where(self.goal_distances < 0.25, 1, 0).to(self._device)


        ## The closer to obstacles, the greater the penalty 
        min_range1 = torch.where(torch.abs(closest_ranges) < 0.9, -1, 0)
        min_range2 = torch.where(torch.abs(closest_ranges) < 0.85, -2, 0)
        min_range3 = torch.where(torch.abs(closest_ranges) < 0.8, -2, 0)
        ...
        v = self._kayas.get_velocities()[:,:2]
        base_velocities = torch.norm(v, p=2, dim=-1)        
        ...
        rewards += speed * 0.02
        rewards += progress_dis*500 #10
        rewards += 0.1 * progress_reward
        rewards += 30 * self.goal_reached
        rewards -= 1 * abs(self.headings)
        #rewards -= 2*self.progress_buf
        #rewards += base_velocities
